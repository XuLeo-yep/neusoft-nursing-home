  ...
  <div class="page-container">
    ...
    <el-card class="table-card" shadow="never">
      <el-table :data="checkOutList" stripe style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="plannedCheckOutDate" label="计划退住时间" width="140" />
    ...
    <el-dialog v-model="auditVisible" title="退住审核" width="500px">
      <el-form :model="auditForm" label-width="100px">
        ...
        <el-form-item label="退住原因"><span>{{ auditForm.reason }}</span></el-form-item>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await request.get('/application/checkout/list')
    let list = res.data || []
...
const handleAuditSubmit = async () => {
  try {
    await request.put('/application/checkout/' + auditForm.id + '/approve', { result: auditForm.result, approver: localStorage.getItem('username') || 'admin' })
